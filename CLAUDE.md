...
**Versioning**: The project uses Calendar Versioning (CalVer) with format `v0.YYMM.PATCH` (e.g., v0.2510.0 for October 2025). The `v0` prefix ensures Go modules compatibility.
...
2. **Generates CalVer version** using `scripts/generate-version.sh` (format: `v0.YYMM.PATCH`)
3. **Updates VERSION file** and Helm chart version/appVersion in Chart.yaml and values.yaml
...
6. **Builds and pushes Docker image** with multi-arch support (amd64, arm64)
   - Tagged as both `0.YYMM.PATCH` and `latest`
7. **Packages Helm chart** with the new version
8. **Creates Git tag** (`v0.YYMM.PATCH`) and commits version changes
